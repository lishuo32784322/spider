...
from tools.verify_ip import verify_ip
...
class RedBookSpiderMiddleware(object):
...
class RedBookDownloaderMiddleware(object):
    # Not all methods need to be defined. If a method is not defined,
    ...
    def process_response(self, request, response, spider):
        if int(response.status) == 461:
            print('重新请求%s:::::' % response.status, response.url)
            return request
